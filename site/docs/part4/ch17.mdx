...
사고의 나무(Tree of Thoughts, ToT)는 연쇄 사고(CoT)를 기반으로 구축된 추론 기법입니다. 이를 통해 대규모 언어 모델은 트리 구조를 형성하는 서로 다른 중간 단계로 분기하여 여러 추론 경로를 탐색할 수 있습니다(<CrossRef id="img-p266-0" label="그림 2" /> 참조). 이 접근법은 역추적(backtracking), 자기 수정, 대안 솔루션 탐색을 가능하게 하여 복잡한 문제 해결을 지원합니다. 가능성의 트리를 유지함으로써 모델은 최종 답변을 확정하기 전에 다양한 추론 궤적을 평가할 수 있습니다. 이 반복적 과정은 전략적 계획과 의사 결정이 필요한 어려운 작업을 처리하는 모델의 능력을 향상시킵니다.

<Diagram id="img-p266-0" />
...
ReAct(추론 및 행동, Reasoning and Acting)는 연쇄 사고(CoT) 프롬프팅과 에이전트가 도구를 통해 외부 환경과 상호작용하는 능력을 통합하는 패러다임입니다(<CrossRef id="img-p271-0" label="그림 3" /> 참조. KB는 지식 베이스를 나타냅니다). 최종 답변을 생성하는 생성 모델과 달리, ReAct 에이전트는 취해야 할 행동에 대해 추론합니다. 이 추론 단계는 CoT와 유사한 내부 계획 과정을 포함하며, 에이전트는 다음 단계를 결정하고, 사용 가능한 도구를 고려하며, 결과를 예측합니다. 이어서 에이전트는 데이터베이스 쿼리, 계산 수행, API와의 상호작용 등 도구 또는 함수 호출을 실행하여 행동합니다.

<Diagram id="img-p271-0" />
...
MASS는 프롬프트와 토폴로지 최적화를 교차하여 복잡한 설계 공간을 체계적으로 탐색하는 다단계 최적화 전략을 채택합니다(<CrossRef id="img-p274-0" label="그림 4" /> 참조).
...
"Deep Research"라는 용어는 지칠 줄 모르는 체계적인 연구 보조원 역할을 하도록 설계된 AI 에이전틱 도구의 범주를 설명합니다. 이 분야의 주요 플랫폼으로는 Perplexity AI, Google의 Gemini 연구 기능, ChatGPT 내 OpenAI의 고급 기능이 있습니다(<CrossRef id="img-p276-0" label="그림 5" /> 참조).

<Diagram id="img-p276-0" />
