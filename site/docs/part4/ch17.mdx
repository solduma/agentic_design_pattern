...
<figcaption>그림 2: 사고의 나무(ToT) — 입력에서 여러 추론 경로가 분기되어 최적 경로를 선택해 답변 도출</figcaption>
</figure>
...
<figure>
<img src="/img/diagrams/img-p272-0.png" alt="ReAct(추론 및 행동) — 프롬프트에서 ReAct 루프를 통해 공개/비공개 지식 베이스를 조회하는 다이어그램" />
<figcaption>그림 3: ReAct(추론 및 행동) — 프롬프트에서 ReAct 루프를 통해 공개/비공개 지식 베이스를 조회하여 답변 생성</figcaption>
</figure>
...
<figcaption>그림 7: 추론 설계 패턴 — 에이전트가 사고(Thinking) 모듈과 상호작용하며 출력 생성</figcaption>
</figure>
