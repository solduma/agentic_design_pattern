...
A2A는 Anthropic의 Model Context Protocol(MCP)을 보완하는 프로토콜입니다(<CrossRef id="img-p237-0" label="그림 1" /> 참조). MCP가 에이전트의 컨텍스트 구조화와 외부 데이터 및 도구와의 상호작용에 초점을 맞추는 반면, A2A는 에이전트 간의 조율과 통신을 촉진하여 태스크 위임과 협업을 가능하게 합니다.

<Diagram id="img-p237-0" />
