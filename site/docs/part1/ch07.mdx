...
<Diagram id="img-p115-0" caption="그림 1 — 멀티 에이전트 시스템의 예시" alt="여러 전문화된 에이전트들이 공동 목표를 위해 협력하는 멀티 에이전트 시스템 구성도" />
...
<Diagram id="img-p118-0" caption="그림 2 — 에이전트들은 다양한 방식으로 통신하고 상호작용합니다." alt="단일 에이전트부터 네트워크, 감독자, 계층형, 커스텀까지 다양한 에이전트 상호작용 모델을 보여주는 다이어그램" />
...
<Diagram id="img-p130-0" caption="그림 3 — 멀티 에이전트 디자인 패턴" alt="멀티 에이전트 디자인 패턴의 시각적 요약: 전문화된 에이전트들이 역할을 분담하여 복잡한 목표를 달성하는 구조" />
